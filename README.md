...
**Другие типы, с которыми вы постоянно будете сталкиваться:**
...
**var** - указывает, что тип переменной будет определен позже.
...
```
Преимущество константных конструкторов в том, что мы можем создать константный объект и в случае полного совпадения параметров объект будет находится в одной ячейки памяти **(ДАННОЕ ОПРЕДЕЛЕНИЕ ПРОХОДИТ ПРОВЕРКУ ПОДЛИННОСТИ)**
